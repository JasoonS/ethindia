...
class PriceSection extends Component<{ contracts: any }> {
  ...
  async updateTimeHeld(props: any, timeHeldKey: any) {
    ...
    let currentTimeHeld =
      ...
      (date.getTime() / 1000 - parseInt(this.getTimeAcquired(props)));
    ...
    */
    if (
      ...
      this.contracts.VitalikSteward.address
    ) {
      const deployedtime = new this.utils.BN('1553202847');
      currentTimeHeld = new this.utils.BN(currentTimeHeld)
        .sub(deployedtime)
        .toString();
    }
  ...
  getArtworkPrice(props: any) {
    return new this.utils.BN(
      props.contracts['VitalikSteward']['price']['0x0'].value
    );
  }
  ...
  getTimeAcquired(props: any) {
    return props.contracts['VitalikSteward']['timeAcquired'][
      this.state.timeAcquiredKey
    ].value;
  }
  ...
  render() {
    return (
      <Fragment>
        <h2>
          Valued at:{' '}
          <ContractData contract='VitalikSteward' method='price' toEth /> ETH
          (~${this.state.USD} USD){' '}
        </h2>
        Current Patron:{' '}
        <ContractData
